...
import type { ContactPageData, ContactHeroSection, ContactNewsletterSection, ContactFormSection, ContactInfoSection } from "@/sanity/queries/contactPage";
...
const FALLBACK_MAPS_URL = "https://www.google.com/maps/place/The+Hive+Community+Circle/@34.044254,-81.0319489,17z/data=!3m1!4b1!4m6!3m5!1s0x88f8bb73a2107003:0x3018e4f7f747e058!8m2!3d34.044254!4d-81.029374!16s%2Fg%2F11h0mwc9st?entry=ttu&g_ep=EgoyMDI2MDMxMS4wIKXMDSoASAFQAw%3D%3D";
...
const FALLBACK_NEWSLETTER_URL = "https://thehivecc.dm.networkforgood.com/emails/first_name-hope-is-growing-in-south-carolina-thanks-to-you-9bd6cd6f-d221-4744-a983-fa7ee063e49a";
...
export default function ContactClient({
  ...
  const newsletterSec = sections.find((s): s is ContactNewsletterSection => s._type === "sectionContactNewsletter");
  ...
  const heroEyebrow = heroSec?.eyebrow ?? copy.contact.heroEyebrow;
  const heroTitle = heroSec?.title ?? copy.contact.heroTitle;
  const heroBody = heroSec?.body ?? copy.contact.heroBody;

  const newsletterTitle = newsletterSec?.title ?? copy.contact.newsletterTitle;
  const newsletterFormTitle = newsletterSec?.formTitle ?? copy.contact.newsletterFormTitle;
  const newsletterEmailLabel = newsletterSec?.emailLabel ?? copy.contact.newsletterEmail;
  const newsletterSubmitLabel = newsletterSec?.submitLabel ?? copy.contact.newsletterSubmit;
  const newsletterUrl = newsletterSec?.newsletterUrl ?? siteSettings?.newsletterUrl ?? FALLBACK_NEWSLETTER_URL;

  // Contact form labels
  const formTitle = formSec?.formTitle ?? copy.contact.formTitle;
  const formDescription = formSec?.formDescription ?? "Choose the topic that best matches your message so we can route it to the right member of the team faster.";
  const nameLabel = formSec?.nameLabel ?? copy.contact.fields.name;
  const namePlaceholder = formSec?.namePlaceholder ?? copy.contact.fields.placeholders.name;
  const emailFieldLabel = formSec?.emailLabel ?? copy.contact.fields.email;
  const emailPlaceholder = formSec?.emailPlaceholder ?? copy.contact.fields.placeholders.email;
  const phoneLabel = formSec?.phoneLabel ?? copy.contact.fields.phone;
  const phonePlaceholder = formSec?.phonePlaceholder ?? copy.contact.fields.placeholders.phone;
  const subjectLabel = formSec?.subjectLabel ?? copy.contact.fields.subject;
  const subjectPlaceholder = formSec?.subjectPlaceholder ?? copy.contact.fields.placeholders.subject;
  const commentLabel = formSec?.commentLabel ?? copy.contact.fields.comment;
  const commentPlaceholder = formSec?.commentPlaceholder ?? copy.contact.fields.placeholders.comment;
  const submitLabel = formSec?.submitLabel ?? copy.contact.fields.submit;
  const resolvedSubjectOptions =
    formSec?.subjectOptions?.length
      ? formSec.subjectOptions.filter((o) => o.value && o.label).map((o) => ({ value: o.value!, label: o.label! }))
      : copy.contact.fields.subjectOptions;

  // Contact info labels
  const infoTitle = infoSec?.infoTitle ?? copy.contact.infoTitle;
  const infoDescription = infoSec?.infoDescription ?? "Prefer to reach out directly? You can call, email, or connect with us on social media.";
  const stayConnectedLabel = infoSec?.stayConnectedLabel ?? "Stay Connected";
  const infoEmailLabel = infoSec?.emailLabel ?? copy.contact.info.email;
  const infoPhoneLabel = infoSec?.phoneLabel ?? copy.contact.info.phone;
  const infoAddressLabel = infoSec?.addressLabel ?? copy.contact.info.address;

  // Contact info from siteSettings with fallbacks
            ...
            <FieldSet className="rounded-[2rem] border border-hive-blue/10 bg-gradient-to-br from-hive-blue via-cyan-700 to-teal-600 p-6 shadow-[0_24px_60px_-24px_rgba(7,89,133,0.75)] sm:p-8">
              <FieldGroup className="gap-6">
                <Field>
                  ...
                  <Input id="name" name="name" autoComplete="off" required placeholder={namePlaceholder} className="h-12 rounded-2xl border-white/15 bg-white/96 px-4 text-base text-slate-900 placeholder:text-slate-500 focus-visible:border-hive-orange focus-visible:ring-hive-orange/30" />
                </Field>
                <Field>
                  <FieldLabel htmlFor="email" className="text-sm font-semibold uppercase tracking-[0.2em] text-white/80">
                    {emailFieldLabel}
                  </FieldLabel>
                  <Input id="email" name="email" type="email" required autoComplete="off" placeholder={emailPlaceholder} className="h-12 rounded-2xl border-white/15 bg-white/96 px-4 text-base text-slate-900 placeholder:text-slate-500 focus-visible:border-hive-orange focus-visible:ring-hive-orange/30" />
                </Field>
                <Field>
                  <FieldLabel htmlFor="phone" className="text-sm font-semibold uppercase tracking-[0.2em] text-white/80">
                    {phoneLabel}
                  </FieldLabel>
                  <Input id="phone" name="phone" type="tel" autoComplete="off" placeholder={phonePlaceholder} className="h-12 rounded-2xl border-white/15 bg-white/96 px-4 text-base text-slate-900 placeholder:text-slate-500 focus-visible:border-hive-orange focus-visible:ring-hive-orange/30" />
                </Field>
                <Field>
                  ...
                  <Select name="subject" required defaultValue={defaultSubject}>
                    <SelectTrigger id="subject" className="h-12 w-full rounded-2xl border-white/15 bg-white/96 px-4 text-left text-base text-slate-900 shadow-none data-[placeholder]:text-slate-500 focus-visible:border-hive-orange focus-visible:ring-hive-orange/30">
                ...
                <Field>
                  ...
                  <Textarea id="comment" name="comment" required autoComplete="off" placeholder={commentPlaceholder} className="min-h-36 rounded-2xl border-white/15 bg-white/96 px-4 py-3 text-base text-slate-900 placeholder:text-slate-500 focus-visible:border-hive-orange focus-visible:ring-hive-orange/30" />
                </Field>
                <Button type="submit" className="h-13 rounded-2xl bg-hive-orange text-lg font-semibold text-white shadow-[0_14px_28px_-14px_rgba(249,115,22,0.9)] transition-transform hover:-translate-y-0.5 hover:bg-hive-orange/90">
                  {submitLabel}
                </Button>
              </FieldGroup>
            </FieldSet>
          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_22px_55px_-30px_rgba(15,23,42,0.35)] sm:p-8">
            ...
            <div className="mt-8 flex flex-col gap-6">
              <div className="flex items-start gap-4 rounded-2xl bg-slate-50 p-4">
                ...
                <div>
                  <p className="mb-1 text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">{infoEmailLabel}</p>
              ...
              <div className="flex items-start gap-4 rounded-2xl bg-slate-50 p-4">
                ...
                <div>
                  <p className="mb-1 text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">{infoPhoneLabel}</p>
              ...
              <div className="flex items-start gap-4 rounded-2xl bg-slate-50 p-4">
                ...
                <div>
                  <p className="mb-1 text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">{infoAddressLabel}</p>
                  <Link href={mapsUrl} target="_blank" rel="noopener noreferrer" className="text-lg font-semibold leading-8 text-slate-800 transition-colors hover:text-hive-yellow">
            ...
            <div className="mt-8 border-t border-slate-200 pt-6">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">{stayConnectedLabel}</p>
