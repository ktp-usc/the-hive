...
const FALLBACK_FOUNDER: FounderMember[] = [{
  ...
  sparkBody: "As a survivor of sexual abuse and a native of South Carolina, Ashley Olayinka recognized the critical gaps in culturally responsive support for Black and Brown women and girls impacted by gender-based violence.",
  visionTitle: "Vision Today",
  visionBody: "Today, Ashley leads The Hive alongside fellow survivors, working to decrease barriers and expand access to equitable, trauma-informed, and economically empowering services.",
  profileBody: "Ashley Olayinka is a transformative leader, healing justice advocate, and founder of The Hive.",
}];

const FALLBACK_TEAM: TeamMemberSanity[] = [
  { _id: "t1", name: "Alyson Berry",               role: "Executive Administrator",            imageUrl: "/member-images/AlysonBerry.avif",               email: "alyson@thehivecc.org",         extension: "ext 104" },
  { _id: "t2", name: "Jalona Webb",                 role: "Legal Outreach Advocate",            imageUrl: "/member-images/JalonaWebb.avif",                email: "jalona.webb@thehivecc.org",    extension: "ext 108" },
  { _id: "t3", name: "Kinnethia Tolson",            role: "Education and Volunteer Coordinator",imageUrl: "/member-images/KinnethiaTolson.avif",           email: "kinnethia@thehivecc.org",      extension: "ext. 109" },
  { _id: "t4", name: "Beatrice Hernandez-Morales",  role: "Bilingual Outreach Advocate",        imageUrl: "/member-images/BeatriceHernandezMorales.avif",  email: "beatrice@thehivecc.org",       extension: "ext. 107" },
];
...
export default async function AboutPage() {
  ...
  const whyBody         = imageText?.body     ?? "We were founded in 2015 with a visionary spirit and urgent objective: to help prevent violence against some of our nation's most vulnerable populations of women and girls.";
  ...
  const joinBody        = joinSection?.body    ?? "Send your resume and a cover letter to hello@thehivecc.org.";
  ...
  const founderMembers: FounderMember[]    = founderSec?.members?.length ? founderSec.members : FALLBACK_FOUNDER;
  const teamMembers: TeamMemberSanity[]    = teamSec?.members?.length    ? teamSec.members    : FALLBACK_TEAM;
  const boardMembers: BoardMemberSanity[]  = boardSec?.members?.length   ? boardSec.members   : FALLBACK_BOARD;
    ...
    <main className="site-page">
      <div className="site-page--narrow space-y-10">
        ...
        <AboutTabs
          ...
          teamSection={{ label: teamSec?.groupLabel ?? "Team Members", eyebrow: teamSec?.eyebrow }}
          boardSection={{ label: boardSec?.groupLabel ?? "Board of Directors", eyebrow: boardSec?.eyebrow }}
