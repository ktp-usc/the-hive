...
function resolveCategoryKey(groupLabel?: string, category?: string | null): CategoryKey | null {
  ...
  const normalizedValue = normalizeValue(category) ?? normalizeValue(groupLabel);
  ...
  switch (normalizedValue) {
    ...
    case "nonprofit":
    case "nonprofitorganizations":
    case "organizacionessinfinesdelucro":
      return "nonprofit";
    case "lawenforcement":
    case "fuerzapublica":
    case "seguridadpublica":
      return "lawEnforcement";
    case "education":
    case "educacion":
      return "education";
    case "faithbased":
    case "comunidadesdefe":
      return "faithBased";
    case "merchant":
    case "merchantbased":
    case "comercios":
    ...
    partnerSections.length > 0
      ? partnerSections
          .map((section) => {
            ...
            const key = resolveCategoryKey(section.groupLabel, partners[0]?.category);
            const translatedLabel =
              key != null ? copy.partners.categoryTitles[key] : section.groupLabel ?? "";

            return {
              key: section._key ?? translatedLabel,
              label: useCmsText ? section.groupLabel ?? translatedLabel : translatedLabel,
              partners: partners.map(({ src, alt }) => ({ src, alt })),
            };
          })
          .filter((section): section is NonNullable<typeof section> => Boolean(section))
      : STATIC_CATEGORIES.map((category) => ({
          key: category.key,
          label: copy.partners.categoryTitles[category.key],
      ...
      <div className="site-page--narrow space-y-10">
        ...
        <section className="site-surface px-6 py-8 sm:px-10 sm:py-10 lg:px-14">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="site-copy mt-4">{copy.partners.heroBody}</p>
          </div>
          ...
          <div className="mx-auto mt-12 grid max-w-6xl items-center gap-8 md:grid-cols-2">
            <div className="relative h-72 w-full sm:h-96 md:h-full">
              <Image
                src={beeBox?.imageUrl ?? "/partner-images/TheBeeBox.avif"}
                alt={copy.partners.beeBoxTitle}
                fill
                className="object-contain"
                priority
              />
            ...
            <div>
              <h3 className="text-2xl font-semibold text-hive-blue">{copy.partners.beeBoxTitle}</h3>
              <p className="site-copy mt-4">
                {useCmsText && beeBox?.body ? beeBox.body : copy.partners.beeBoxBody}
              </p>
            </div>
          </div>
          ...
          <div className="mx-auto mt-16 max-w-2xl text-center">
            <p className="site-copy text-center">
              ...
              <a className="site-link" href="mailto:volunteer@thehivecc.org">
        ...
        <div className="mt-12 space-y-8 pb-12">
          {partnerCategories.map((category, index) => (
            <Card key={category.key} className="overflow-hidden border border-gray-200 shadow-sm">
              ...
              {index < partnerCategories.length - 1 ? <Separator /> : null}
            </Card>
          ))}
        </div>
      </div>
